...
*/
    ...
    <body onload='oldProblem({{$id}});$("#callTime").val(getTime());$("#callDate").val(getDate());' bgcolor="#f2f2f2">
    ...
    function openLog () {
		...
		$("#filterSearchButton").bind("click", function () {
			...
			if(filteredProblems.length == 0) {
				problemDiv.append("<h3 style='margin: 10px 10px 10px 10px';>No results found</h3>");
			}
			for(var i=0;i<filteredProblems.length;i++) {
		...
		*/
		...
		$(".problemForm").each(function () {
			...
			if(data.new) {
				...
				newData.created = ""; // creation date will not change
